...
async def calculate_collection_time(
    ...
    available_collection_time = await bakery_service.calculate_collection_time(
        bakery_id=bakery_id, product_id=product_id)
    return {'success': True,
            'message': 'Collection time calculated successfully.',
            'data': available_collection_time}

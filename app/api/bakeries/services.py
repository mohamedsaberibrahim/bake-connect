...
class BakeryService:
    ...
    async def calculate_collection_time(self, bakery_id: int, product_id: int):
        ...
        result = find_available_time_slots(
            busy_slots, datetime(2023, 10, 19, 12, 0), datetime(2023, 10, 19, 23, 59))

        [product] = await self.product_dao.filter(id=product_id)
        available_collection_time = get_available_ready_times(
            result, product.baking_time)
